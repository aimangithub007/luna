...
if not _HERMES_AVAILABLE:
    ...
    DEFAULT_CONFIG = {}
    OPTIONAL_ENV_VARS = {}
    def cfg_get(*args, **kw): return None
    def get_config_path(*args, **kw): return _P("/tmp/hermes")
    def get_env_path(*args, **kw): return _P("/tmp/hermes")
    def get_hermes_home(*args, **kw): return _P("/tmp/hermes")
    def load_config(*args, **kw): return {}
    def load_env(*args, **kw): return {}
    def save_config(*args, **kw): pass
    def save_env_value(*args, **kw): pass
    def remove_env_value(*args, **kw): pass
    def check_config_version(*args, **kw): pass
    def redact_key(k, v): return v
    def get_running_pid(*args, **kw): return None
    def read_runtime_status(*args, **kw): return None
...
# ---------------------------------------------------------------------------
# Load environment variables from ~/.hermes/.env into os.environ at module level
# so that os.getenv() calls below pick up user-defined keys.
try:
    _env_on_disk = load_env()
    for _k, _v in _env_on_disk.items():
        if _k not in os.environ:
            os.environ[_k] = _v
    # Force HERMES_HOME to match the env on disk if it was loaded from a specific profile
    if "HERMES_HOME" in _env_on_disk:
         os.environ["HERMES_HOME"] = _env_on_disk["HERMES_HOME"]
except Exception:
    pass

_SESSION_TOKEN = os.getenv("HERMES_DASHBOARD_TOKEN", "").strip() or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Hermes-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``hermes dashboard --tui``
# or HERMES_DASHBOARD_TUI=1.  Set from :func:`start_server`.
_DASHBOARD_EMBEDDED_CHAT_ENABLED = True   # Custom: enabled for standalone dashboard
...
_PUBLIC_API_PATHS: frozenset = frozenset({
    ...
    "/api/auth/login",
    "/api/profiles",  # Allow reading profiles for switcher
})
...
def _has_valid_session_token(request: Request) -> bool:
    ...
    session_query = request.query_params.get("token", "")
    if session_query and hmac.compare_digest(
        session_query.encode(),
        _SESSION_TOKEN.encode(),
    ):
        return True

    # 3. Check Authorization Header (Legacy)
...
def _require_token(request: Request) -> None:
    ...
    if not _has_valid_session_token(request):
        # DEBUG log for the developer
        import sys
        header_val = request.headers.get(_SESSION_HEADER_NAME, "MISSING")
        print(f"DEBUG: Auth Fail. Expected: {_SESSION_TOKEN[:3]}... Received: {header_val[:3]}...", file=sys.stderr)
...
# ── 9Router Bridge ────────────────────────────────────────────────────────
_ROUTER9_URL = os.getenv("ROUTER9_URL", "http://localhost:20128").rstrip("/")

async def _fetch_9router(path: str) -> dict:
    """Helper to fetch JSON from 9Router API with safety checks."""
    url = f"{_ROUTER9_URL}{path}"
    loop = asyncio.get_running_loop()
    try:
        def _do_fetch():
            ...
            with urllib.request.urlopen(req, timeout=3.0) as resp:
                ...
                return {"error": "router9_error", "status": resp.status}
        return await loop.run_in_executor(None, _do_fetch)
    except Exception as e:
        # Return a clean error if 9Router is offline or unreachable
        return {"error": "router9_unavailable", "detail": str(e)}

from fastapi.responses import StreamingResponse

@app.get("/api/9router/usage/stream")
async def stream_9router_usage():
    import httpx
    async def event_stream():
        url = f"{_ROUTER9_URL}/api/usage/stream"
        try:
...
@app.get("/api/9router/usage/stats")
async def get_9router_stats(period: str = "7d"):
    return await _fetch_9router(f"/api/usage/stats?period={period}")

@app.get("/api/9router/providers")
async def get_9router_providers():
    return await _fetch_9router("/api/providers")

@app.get("/api/9router/usage/chart")
async def get_9router_chart(period: str = "7d"):
    return await _fetch_9router(f"/api/usage/chart?period={period}")

@app.get("/api/9router/keys")
async def get_9router_keys():
    return await _fetch_9router("/api/keys")

@app.get("/api/9router/settings")
async def get_9router_settings():
    return await _fetch_9router("/api/settings")
...
async def use_profile_endpoint(name: str):
    """Set a profile as the sticky default (~/.hermes/active_profile)."""
    from hermes_cli import profiles as profiles_mod
    try:
        canon = profiles_mod.normalize_profile_name(name)
        profiles_mod.validate_profile_name(canon)
        if not profiles_mod.profile_exists(canon):
            raise HTTPException(status_code=404, detail=f"Profile {canon} not found")
        
        active_path = profiles_mod._get_active_profile_path()
        active_path.write_text(canon, encoding="utf-8")
        return {"ok": True, "name": canon}
    except Exception as e:
        _log.exception("POST /api/profiles/%s/use failed", name)
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_dashboard_commands():
    """Parse and return commands from HERMES_COMMANDS.md and documentation."""
    import re
    sections = []
    
    # 1. Parse HERMES_COMMANDS.md
    commands_path = PROJECT_ROOT.parent / "HERMES_COMMANDS.md"
    if not commands_path.exists():
        commands_path = PROJECT_ROOT / "HERMES_COMMANDS.md"
        ...
        try:
            ...
            for line in content.splitlines():
                ...
                if line.startswith("## "):
                    ...
                    if len(parts) == 2:
                        cmd_part, desc_part = parts
                        cmd_name = re.sub(r'[`*]+', '', cmd_part).strip()
                        current_section["commands"].append({"name": cmd_name, "desc": desc_part.strip()})
        except Exception as e:
            _log.error("Error parsing HERMES_COMMANDS.md: %s", e)

    # 2. Parse Documentation (CLI & Slash)
    docs_ref = PROJECT_ROOT / "website" / "docs" / "reference"
    _log.info("Commands API: docs_ref=%s", docs_ref)
    for doc_file in ["cli-commands.md", "slash-commands.md", "profile-commands.md"]:
        path = docs_ref / doc_file
        _log.info("Commands API: checking %s (exists=%s)", path, path.exists())
        if path.exists():
            try:
                content = path.read_text(encoding="utf-8")
                h2_header = "CLI Reference" if doc_file == "cli-commands.md" else ("Slash Reference" if doc_file == "slash-commands.md" else "Profile Reference")
                current_section = {"name": h2_header, "commands": []}
                
                header_context = None
                for line in content.splitlines():
                    line = line.strip()
                    if line.startswith("## "):
                        header_context = line[3:].strip()
                        ...
                        if len(parts) >= 3:
                            ...
                            if cmd_part.startswith("`") and cmd_part.endswith("`"):
                                ...
                                if cmd_name.lower() in ("command", "option", "---"): continue
                                
                                # Skip very short/meta things
                                if len(cmd_name) < 2: continue
                                
                                name_display = cmd_name
                                if header_context and header_context not in h2_header:
                                    # Don't add context if it's too repetitive
                                    pass
                ...
                _log.error("Error parsing %s: %s", doc_file, e)
                
    return {"sections": sections}


def parse_sidebars_ts(content: str):
    """Simple regex-based parser for docusaurus sidebars.ts"""
    import re
    
    # We follow the 'docs' list in the exported sidebars object
    # Normalize: remove comments
    content = re.sub(r'//.*', '', content)
    content = re.sub(r'/\*.*?\*/', '', content, flags=re.DOTALL)
    
    # Find the docs array: docs: [ ... ] (bracket aware)
    start_match = re.search(r'docs:\s*\[', content)
    if not start_match:
        return []
    ...
    docs_raw = docs_raw[1:-1] # Remove outer [ ]
    
    # Extract items manually to handle objects and strings
    items = []
    current = ""
    depth = 0
    in_str = False
    ...
    for char in docs_raw:
        if not in_str:
            ...
            if char == str_char:
                # Handle escaped quotes defensively
    ...
    for item in items:
        if item.startswith('{'):
            # It's an object, assume it's a category
            label_m = re.search(r'label:\s*[\'"](.*?)[\'"]', item)
            
            # More robustly find the items array [ ... ]
            items_match = re.search(r'items:\s*\[', item)
            if items_match:
                start_idx = items_match.end() - 1
                bracket_depth = 0
                items_str = ""
                for char in item[start_idx:]:
                    if char == '[': bracket_depth += 1
                    elif char == ']': bracket_depth -= 1
                    items_str += char
                    if bracket_depth == 0: break
                
                # Extract all strings from inside the items array
                sub_items = re.findall(r'[\'"](.*?)[\'"]', items_str)
...
async def get_docs_topics():
    """Parse website/sidebars.ts and return the documentation structure."""
    sidebars_path = PROJECT_ROOT / "website" / "sidebars.ts"
    if not sidebars_path.exists():
        _log.warning(f"sidebars.ts not found at {sidebars_path}")
        return {"sections": []}
    
    try:
        content = sidebars_path.read_text(encoding="utf-8")
        sections = parse_sidebars_ts(content)
        return {"sections": sections}
    except Exception as e:
        _log.error(f"Failed to parse sidebars.ts: {e}")
        return {"sections": [], "error": str(e)}

@app.get("/api/docs/content/{doc_path:path}")
async def get_doc_content(doc_path: str):
    """Read a markdown file from website/docs/."""
    docs_dir = PROJECT_ROOT / "website" / "docs"
    
    # Try common extensions
    for ext in ["", ".md", ".mdx"]:
        target = docs_dir / f"{doc_path}{ext}"
        if target.is_file():
            break
        # Try as directory index
        target_idx = docs_dir / doc_path / f"index{ext}"
        if target_idx.is_file():
            target = target_idx
            break
    else:
    ...
    try:
        ...
        # Strip MDX imports and components
...
async def search_docs(q: str = ""):
    """Simple fuzzy search for documents."""
    if not q:
        return {"results": []}
    ...
    for p in docs_dir.rglob("*"):
        if p.suffix in {".md", ".mdx"}:
            ...
            doc_id = rel_path.replace(".mdx", "").replace(".md", "").replace("\\", "/")
            if doc_id.endswith("/index"):
                doc_id = doc_id[:-6]
                
            title = p.stem.replace("-", " ").title()
            try:
                # Quick scan for title in frontmatter
                with open(p, "r", encoding="utf-8") as f:
                    count = 0
                    for line in f:
                        count += 1
                        if count > 20: break # Don't scan huge files entirely
                        if line.startswith("# "):
                            title = line[2:].strip()
                            break
            ...
            # Rank 1: Starts with title match
            # Rank 2: Contains title match
            # Rank 3: Exact command/header match
            # Rank 4: Starts with command/header match
            # Rank 5: Contains command/header match
            
            if q in title.lower() or q in doc_id.lower():
                results.append({
                    "id": doc_id,
                    "title": title,
                    "path": rel_path,
                    "score": 0 if title.lower() == q else (1 if title.lower().startswith(q) else 2)
                })

            # If it's a reference doc, also scan for individual headers (commands)
            if "reference/" in doc_id and p.suffix in {".md", ".mdx"}:
                try:
                    import re
                    ...
                    for line in file_content.splitlines():
                        if line.startswith("## ") or line.startswith("### "):
                            ...
                            # Skip standard headers like "Examples", "Subcommands", etc. unless they match exactly
                            if q in header_text.lower() and len(header_text) > 3:
                                # slugify header for anchor
                                anchor = re.sub(r'[^a-z0-9]+', '-', header_text.lower()).strip('-')
                                score = 3 if header_text.lower() == q else (4 if header_text.lower().startswith(q) else 5)
                                results.append({
                                    "id": doc_id,
                                    "title": header_text,
                                    "path": rel_path,
    ...
    # Sort by score primarily, then by title alphabetically
    results.sort(key=lambda x: (x.get("score", 10), x["title"]))
    return {"results": results[:10]}


@app.get("/api/docs/skills")
async def get_docs_skills():
    """Returns rich metadata for all skills by parsing SKILL.md files."""
    all_skills = []
    
    # Try to load existing skills.json if it exists and is fresh
    skills_json_path = PROJECT_ROOT / "website" / "src" / "data" / "skills.json"
    if skills_json_path.exists():
        try:
            return {"skills": json.loads(skills_json_path.read_text(encoding="utf-8"))}
        except:
            pass

    # Fallback: Manual extraction (simplified port of extract-skills.py)
    local_dirs = [("skills", "built-in"), ("optional-skills", "optional")]
    cat_labels = {
        "apple": "Apple", "autonomous-ai-agents": "AI Agents", "blockchain": "Blockchain",
        "communication": "Communication", "creative": "Creative", "data-science": "Data Science",
        "devops": "DevOps", "dogfood": "Dogfood", "domain": "Domain", "email": "Email",
        "gaming": "Gaming", "gifs": "GIFs", "github": "GitHub", "health": "Health",
        "inference-sh": "Inference", "leisure": "Leisure", "mcp": "MCP", "media": "Media",
        "migration": "Migration", "mlops": "MLOps", "note-taking": "Note-Taking",
        "productivity": "Productivity", "red-teaming": "Red Teaming", "research": "Research",
        "security": "Security", "smart-home": "Smart Home", "social-media": "Social Media",
        "software-development": "Software Dev", "translation": "Translation", "other": "Other",
    }

    for base_dir, source_label in local_dirs:
        ...
        for root, _, files in os.walk(str(base_path)):
            ...
            try:
                ...
                all_skills.append({
                    ...
                    "category": category,
                    "categoryLabel": cat_labels.get(category, category.replace("-", " ").title()),
                    "source": source_label,
                    "tags": fm.get("tags", []),
                    "platforms": fm.get("platforms", []),
                    "author": fm.get("author", ""),
                    "version": fm.get("version", ""),
                    "link": f"reference/skills-catalog#{category}-{p.parent.name}" # fallback link
                })
            except Exception as e:
                _log.error(f"Error parsing {p}: {e}")

    return {"skills": all_skills}

@app.get("/api/docs/user-stories")
async def get_user_stories():
    """Returns content of userStories.json."""
    p = PROJECT_ROOT / "website" / "src" / "data" / "userStories.json"
    if p.exists():
        try:
            return {"stories": json.loads(p.read_text(encoding="utf-8"))}
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))
    return {"stories": []}
...
async def dashboard_login(request: Request):
    ...
    if not expected_pass:
        # Fallback: cocokkan dengan HERMES_DASHBOARD_TOKEN
        expected_pass = _SESSION_TOKEN
...
TERMINAL_ROOT = Path(__file__).resolve().parent.parent.parent
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    mode = ws.query_params.get("mode") or "tui"
    try:
        argv, cwd, env = _resolve_chat_argv(resume=resume, sidecar_url=sidecar_url, mode=mode)
...
async def get_memory_graph(request: Request):
    """Dynamically scan Hermes architecture to return nodes and edges for 3D UI."""
    import time
    from pathlib import Path
    
    home = get_hermes_home()
    nodes = [{"id": 1, "name": "Orchestrator", "type": "core", "size": 150000}]
    edges = []
    node_idx = 2
    
    # 1. CORE MEMORY
    for path_str in [home / "MEMORY.md", home / "USER.md"]:
        f = Path(path_str)
        if f.exists():
            st = f.stat()
            nodes.append({
                "id": node_idx, "name": f.name, "type": "memory", 
                "size": st.st_size, "modified": f"{int(time.time() - st.st_mtime)}s ago"
            })
            edges.append([1, node_idx])
            node_idx += 1

    # 2. SESSIONS (limit to top 15 recently modified)
    sessions_dir = home / "sessions"
    if sessions_dir.exists():
        session_files = sorted(sessions_dir.glob("session_*.json"), key=lambda x: x.stat().st_mtime, reverse=True)[:5]
        dump_files = sorted(sessions_dir.glob("request_dump_*.json"), key=lambda x: x.stat().st_mtime, reverse=True)[:10]
        
        for f in (session_files + dump_files):
            st = f.stat()
            nodes.append({
                "id": node_idx, "name": f.name, "type": "session", 
                "size": st.st_size, "modified": f"{int(time.time() - st.st_mtime)}s ago"
            })
            edges.append([1, node_idx])
            node_idx += 1
        ...
        for d in skills_dir.iterdir():
            if d.is_dir() and not d.name.startswith("__"):
                try:
                    ...
                    size = 50000
                nodes.append({
                    "id": node_idx, "name": f"skills/{d.name}", "type": "skill", 
                    "size": size, "modified": ""
                })
                edges.append([1, node_idx])
                node_idx += 1
...
def start_server(
    host: str = "127.0.0.1",
    port: int = 8119,  # Custom port — tidak konflik dengan hermes dashboard (9119)
